        ...
        <div className="flex items-center justify-between">
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="rounded-full bg-navy px-5 py-2.5 text-xs font-semibold text-white hover:bg-navy-light"
            >
              Mark all read
            </button>
          )}
          ...
          <div className="flex flex-col">
            {visible.map((n) => (
              <button
                ...
                className={clsx(
                  'flex items-start gap-3 rounded-xl px-4 py-3.5 text-left hover:bg-hover',
                  !n.read && 'bg-surface',
                )}
              >
                <div
                ...
                {!n.read && <span className="ml-auto mt-1.5 h-2 w-2 shrink-0 rounded-full bg-brand-blue" />}
              </button>
            ))}
            {visible.length === 0 && <div className="px-4 py-8 text-center text-sm text-muted">No notifications yet.</div>}
          </div>
        </Tile>
      </div>
    </AppShell>
